...
function GroupsView() {
  ...
  const cargarGrupos = useCallback(async () => {
    ...
    try {
      const response = await fetch(`https://52.173.30.244:9000/api/groups/user-groups?uid=${userId}`);
      if (response.ok) {
  ...
  const handleGroupClick = (group) => {
    ...
    fetch(`https://52.173.30.244:9000/api/groups/${group.gid}/members`)
  ...
  const handleAddUserToGroup = async () => {
    ...
    try {
      ...
      const userResponse = await fetch(`https://52.173.30.244:9000/api/users/getuid?username=${newUsername}`);
      if (!userResponse.ok) {
      ...
      const response = await fetch('https://52.173.30.244:9000/api/groups/join', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const userResponse = await fetch(`https://52.173.30.244:9000/api/users/getuid?username=${usernameToDelete}`);
      if (!userResponse.ok) {
        const errorData = await userResponse.json();
        console.error(errorData.error);
        return;
      ...
      const response = await fetch(`https://52.173.30.244:9000/api/groups/remove-member`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        fetch(`https://52.173.30.244:9000/api/groups/${selectedGroup.gid}/members`)
          .then(response => response.json())
          .then(data => setMembers(data.members))
          .catch(error => console.error('Error loading members:', error));
      } else {
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      const response = await fetch(`https://52.173.30.244:9000/api/groups/leave`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteGroup = async () => {
    ...
    try {
      ...
      const userGroupResponse = await fetch(`https://52.173.30.244:9000/api/groups/leave`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const groupResponse = await fetch(`https://52.173.30.244:9000/api/groups/delete`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
